...
void QRKRegister::quickProductButtons(int id)
{
  ...
  query.exec();
  QString backgroundcolor = "#808080";
  if (query.next())
    backgroundcolor = (query.value(0).toString() == "")?backgroundcolor: query.value(0).toString();
  ...
  while (query.next())
  {
    QPushButton* bt = new QPushButton(widget);

    bt->setMinimumHeight(60);
    bt->setMaximumWidth(150);
    QString btText = wordWrap(query.value(1).toString(), bt->width(), bt->font());
    btText = QString("%1\n %2 %3")
        .arg(btText)
    ...
    backgroundcolor = (query.value(3).toString() == "")?backgroundcolor: query.value(3).toString();
...
bool QRKRegister::finishReceipts(int payedBy, int id, bool isReport)
{
  ...
  if (currentReceipt != receiptNum +1) {
    qDebug() << "Datenbank error: currentReceipt != lastreceiptNum +1";
    receiptNum = currentReceipt -1;
  }
  int ok =false;
  receiptNum++;
