...
void FvUpdater::InstallUpdate()
{
    ...
    {
        qWarning() << "Function Name: " << Q_FUNC_INFO << " Unable to download the update: "<<reply->errorString();
        return;
    }
    else
        qInfo() << "Function Name: " << Q_FUNC_INFO << " OK";
...
void FvUpdater::httpUpdateDownloadFinished()
{
    ...
    if(reply->error() == QNetworkReply::NoError)
    {
        ...
        if (reply->error() == QNetworkReply::NoError)
        {
            if (reply->isReadable())
            {
                QString rootDirectory = getRootPath();
                qDebug() << "Function Name: " << Q_FUNC_INFO << " rootDirectory: " << rootDirectory;

                // Write download into File
                QFileInfo fileInfo=reply->url().path();
                QString fileName = rootDirectory + QDir::separator() + fileInfo.fileName();
                qDebug() << "Function Name: " << Q_FUNC_INFO << " Writing downloaded file into "<<fileName;
                ...
                // Retrieve List of updated files (Placed in an extra scope to avoid QuaZIP handles the archive permanently and thus avoids the deletion.)
                {
                    ...
                    if (!zip.open(QuaZip::mdUnzip)) {
                        qWarning() << "Function Name: " << Q_FUNC_INFO << " testRead(): zip.open(): " << zip.getZipError();
                        return;
                    }
                    ...
                    {
                        QString sourceFilePath = rootDirectory + QDir::separator() + updateFiles[i].name;
            ...
            else qWarning() << "Function Name: " << Q_FUNC_INFO << " Error: QNetworkReply is not readable!";
        }
        else
        {
            qWarning()<<"Download errors ocurred! HTTP Error Code:"<<httpstatuscode;
        }
...
{
    qDebug() << "Function Name: " << Q_FUNC_INFO << " Skip update";
        ...
        if (m_xml.isStartElement()) {
            ...
            if (m_xml.name() == "item") {
                ...
                if (attribs.hasAttribute("fervor:platform"))
                {
                    ...
                    {
                        xmlEnclosureUrl = attribs.hasAttribute("url") ? attribs.value("url").toString().trimmed() + "/" + m_userInfo : "";
                        xmlEnclosureVersion = "";
                                               ...
                                               QString xmlEnclosureType)
{
    ...
    if (FVIgnoredVersions::VersionIsIgnored(xmlEnclosureVersion)) {
        qInfo() << "Function Name: " << Q_FUNC_INFO << " Version '" << xmlEnclosureVersion << "' is ignored, too old or something like that.";
...
#else
    qInfo() << "Function Name: " << Q_FUNC_INFO << " " << message;
#endif
}

QString FvUpdater::getRootPath(bool app){
#ifdef __APPLE__
    ...
    if (app)
        return filePath.left(filePath.lastIndexOf("/")) + QDir::separator() + QCoreApplication::applicationName().toLower() + ".app";
    else
...
{
    pathToFinish = pathToFinish.isEmpty() ? getRootPath(true) : pathToFinish;
        ...
        else
        {
            if(fi.suffix()=="oldversion")
                if( !appDir.remove( fi.absoluteFilePath() ) )
                    qWarning() << "Function Name: " << Q_FUNC_INFO << " Error: Unable to clean up file: "<<fi.absoluteFilePath();
        }
...
void FvUpdater::restartApplication()
{
    ...
    arguments << "-r";
#ifdef __APPLE__
    QFile::setPermissions(app, QFile::ReadOwner|QFile::WriteOwner|QFile::ExeOwner|QFile::ReadGroup|QFile::ExeGroup|QFile::ReadOther|QFile::ExeOther);
#endif
...
void FvUpdater::setUserCredentials(QString info)
{
    m_userInfo = info;
}
